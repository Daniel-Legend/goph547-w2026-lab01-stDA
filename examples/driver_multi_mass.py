...
def main():
    ...
    if (
        not os.path.exists("examples/mass_set_0.mat")
        or not os.path.exists("examples/mass_set_1.mat")
        or not os.path.exists("examples/mass_set_2.mat")
    ):
    ...
    # load data set
    for kk in range(3):
        data = loadmat(f"examples/mass_set_{kk}.mat")
        m = data["m"][:, 0]
        xm = data["xm"]
        print()
        print(f"Mass set {kk}")
...
def generate_mass_anomaly_sets():
    ...
    for k in range(3):
        ...
        savemat(f"examples/mass_set_{k}.mat", mdict={"m": mm, "xm": xx})
...
def plot_gravity_potential_and_effect(
    ...
    plt.savefig(f"examples/multi_mass_grid_25_set_{kk}.png", dpi=300)
    ...
    plt.savefig(f"examples/multi_mass_grid_5_set_{kk}.png", dpi=300)
